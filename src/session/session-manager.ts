...
import { withRetry, handleSessionError } from '../utils/error-handler.js';
 ...
 */
export class SessionManager implements IGansAuditorCodexSessionManager {
   ...
   */
  private resolveStateDirectory(): string {
   ...
   * Requirement 3.3: Unique session ID generation
   */
  public generateSessionId(cwd?: string, username?: string): string {
   ...
   */
  private validateSessionState(data: any): data is GansAuditorCodexSessionState {
   ...
   */
  private sanitizeSessionState(data: any, sessionId: string): GansAuditorCodexSessionState {
    const now = Date.now();
    ...
    try {
      ...
      if (error instanceof SessionCorruptionError) {
        ...
        try {
          ...
          this.componentLogger.error(`Failed to recover session ${id}`, recoveryError as Error);
          return null;
        }
      }

      // For other errors, log and return null
      this.componentLogger.error(`Failed to load session ${id}`, error as Error);
      return null;
    }
  }
   ...
   */
  public async createSession(id: string, config: GansAuditorCodexSessionConfig): Promise<GansAuditorCodexSessionState> {
    try {
      ...
      this.componentLogger.error(`Failed to create session ${id}`, error as Error);
  ...
  public async updateSession(session: GansAuditorCodexSessionState): Promise<void> {
    try {
      ...
      const updatedSession: GansAuditorCodexSessionState = {
        ...session,
        updatedAt: Date.now(),
      };
      ...
      this.componentLogger.error(`Failed to update session ${session.id}`, error as Error);
   ...
   */
  public async addAuditToHistory(
    sessionId: string,
    thoughtNumber: number,
    review: any,
    ...
    try {
      ...
      this.componentLogger.error(`Failed to add audit to history for session ${sessionId}`, error as Error);
      throw new SessionPersistenceError(
        sessionId, 
        'add-audit-history', 
        error instanceof Error ? error.message : String(error)
   ...
   */
  public async recordCodexFailure(
    sessionId: string,
    thoughtNumber: number,
    error: Error,
    ...
    try {
      ...
      this.componentLogger.error(`Failed to record Codex failure for session ${sessionId}`, updateError as Error);
   ...
   */
  public async hasRecentCodexFailures(sessionId: string, withinMinutes: number = 10): Promise<boolean> {
    try {
      const session = await this.getSession(sessionId);
      ...
      this.componentLogger.warn(`Failed to check recent Codex failures for session ${sessionId}`, error as Error);
      return false;
    }
  }
   ...
   */
  public async getCodexFailureSummary(sessionId: string): Promise<{
    ...
    try {
      ...
      this.componentLogger.warn(`Failed to get Codex failure summary for session ${sessionId}`, error as Error);
      return {
        totalFailures: 0,
        recentFailures: 0,
        commonErrorTypes: [],
   ...
   */
  public async cleanupSessions(maxAge?: number): Promise<void> {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          ...
          this.componentLogger.warn(`Removing corrupted session file ${file}`, error as Error);
          try {
            await fs.unlink(filePath);
            cleanedCount++;
          } catch (unlinkError) {
            this.componentLogger.error(`Failed to remove corrupted session file ${file}`, unlinkError as Error);
          }
        }
      }
      ...
      this.componentLogger.error('Failed to cleanup sessions', error as Error);
  ...
  private startCleanupTimer(): void {
    ...
    this.cleanupTimer = setInterval(() => {
      this.cleanupSessions().catch(error => {
        this.componentLogger.error('Automatic session cleanup failed', error as Error);
      });
   ...
   */
  public async getAllSessions(): Promise<GansAuditorCodexSessionState[]> {
    try {
      await this.ensureStateDirectory();
      const files = await fs.readdir(this.stateDir);
      const sessions: GansAuditorCodexSessionState[] = [];
   ...
   */
  public async addWorkflowStepResult(
    sessionId: string,
    stepName: string,
    stepResult: WorkflowStepResult,
    thoughtNumber: number
    ...
    try {
      ...
      this.componentLogger.error(`Failed to add workflow step result for session ${sessionId}`, error as Error);
      throw new SessionPersistenceError(
        sessionId, 
        'add-workflow-step', 
        error instanceof Error ? error.message : String(error)
   ...
   */
  public async trackQualityProgression(
    sessionId: string,
    qualityMetrics: QualityProgressionEntry
  ): Promise<void> {
    try {
      const session = await this.getSession(sessionId);
      if (!session) {
      ...
      if (qualityMetrics.completionAnalysis) {
        ...
        session.completionReason = qualityMetrics.completionAnalysis.reason as any;
      }
      ...
      this.componentLogger.error(`Failed to track quality progression for session ${sessionId}`, error as Error);
      throw new SessionPersistenceError(
        sessionId, 
   ...
   */
  public async storePromptContext(
    sessionId: string,
    promptContext: PromptContextData
  ): Promise<void> {
    try {
      const session = await this.getSession(sessionId);
      if (!session) {
      ...
      this.componentLogger.error(`Failed to store prompt context for session ${sessionId}`, error as Error);
      throw new SessionPersistenceError(
        sessionId, 
   ...
   * Requirement 6.3: Create session state persistence for prompt context
   */
  public async getPromptContext(sessionId: string): Promise<PromptContextData | null> {
    try {
      const session = await this.getSession(sessionId);
      ...
      this.componentLogger.error(`Failed to get prompt context for session ${sessionId}`, error as Error);
      return null;
    }
  }
   ...
   * Requirement 6.2: Implement quality progression tracking across iterations
   */
  public async analyzeAuditProgress(sessionId: string): Promise<AuditProgressAnalysis> {
    try {
      const session = await this.getSession(sessionId);
      if (!session) {
        throw new SessionNotFoundError(sessionId);
      ...
      const analysis: AuditProgressAnalysis = {
        sessionId,
        currentLoop: session.currentLoop,
        ...
        recommendedActions: this.generateProgressRecommendations(
          scoreProgression,
          isStagnant,
          averageImprovement,
          session.currentLoop
      ...
      this.componentLogger.error(`Failed to analyze audit progress for session ${sessionId}`, error as Error);
      throw new SessionPersistenceError(
        sessionId, 
        'analyze-audit-progress', 
        error instanceof Error ? error.message : String(error)
      );
    }
  }
  ...
  public async getSessionStatistics(sessionId: string): Promise<SessionStatistics> {
    try {
      ...
      this.componentLogger.error(`Failed to get session statistics for ${sessionId}`, error as Error);
      throw new SessionPersistenceError(
        sessionId, 
  ...
  public async addIteration(sessionId: string, iteration: IterationData): Promise<void> {
    try {
      ...
      this.componentLogger.error(`Failed to add iteration for session ${sessionId}`, error as Error);
      throw new SessionPersistenceError(
        sessionId, 
        'add-iteration', 
        error instanceof Error ? error.message : String(error)
   ...
   */
  private detectProgressStagnation(scoreProgression: Array<{ score: number; thoughtNumber: number }>): boolean {
   ...
   */
  private analyzeWorkflowSteps(workflowHistory: WorkflowHistoryEntry[]): WorkflowStepStatistics {
    ...
    const stepStats: Record<string, { count: number; averageDuration: number }> = {};
  ...
  private calculateCompletionProbability(
    scoreProgression: Array<{ score: number; thoughtNumber: number }>,
    currentLoop: number,
    isStagnant: boolean
  ): number {
   ...
   */
  private calculateScoreSimilarity(scores: number[]): number {
    if (scores.length < 2) {
      return 0;
    }
   ...
   */
  private generateStagnationRecommendation(analysis: AuditProgressAnalysis): string {
   ...
   */
  private calculateAverageStepDuration(workflowHistory: WorkflowHistoryEntry[]): number {
   ...
   */
  private getMostFrequentStep(workflowHistory: WorkflowHistoryEntry[]): string | null {
   ...
   */
  private estimateSessionMemoryUsage(session: GansAuditorCodexSessionState): number {
    try {

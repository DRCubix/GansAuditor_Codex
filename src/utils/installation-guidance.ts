...
export class InstallationGuidanceSystem {
  private readonly componentLogger = createComponentLogger('installation-guidance');
   ...
   */
  async generateGuidance(): Promise<InstallationGuidance> {
    ...
    const guidance: InstallationGuidance = {
   ...
   */
  async checkVersionCompatibility(): Promise<VersionInfo> {
    ...
    try {
      ...
      this.componentLogger.debug('Version check failed', { error: (error as Error).message });
   ...
   */
  async generateUpgradeGuidance(): Promise<{
   ...
   */
  async createTroubleshootingGuide(): Promise<{
    commonIssues: Array<{
      issue: string;
      symptoms: string[];
      solutions: string[];
   ...
   */
  private async checkCodexVersion(): Promise<VersionInfo> {
   ...
   */
  private async detectInstallationIssues(): Promise<string[]> {
    const issues: string[] = [];
   ...
   */
  private getVerificationSteps(): string[] {
    return [
   ...
   * Get upgrade steps
   */
  private getUpgradeSteps(targetVersion: string): string[] {
    const steps = [
    ...
    switch (this.platformInfo.platform) {
      case 'darwin':
        steps.push(
          'Update using Homebrew: brew upgrade codex',
          'Or download new installer and run it',
        );
        break;
      case 'linux':
        steps.push(
          'Update using package manager: sudo apt update && sudo apt upgrade codex',
          'Or download new binary and replace existing installation',
        );
        break;
      case 'win32':
        steps.push(
          'Download new Windows installer',
          'Run installer (it will upgrade existing installation)',
        );
        break;
      default:
        steps.push(
          'Download new version from official website',
          'Replace existing installation',
        );
    }

    steps.push(
      'Verify upgrade: codex --version',
   ...
   * Get rollback steps
   */
  private getRollbackSteps(): string[] {
    return [
   ...
   * Get upgrade risks
   */
  private getUpgradeRisks(): string[] {
    return [
 ...
 */
export async function generateUpgradeGuidance(): Promise<{
  currentVersion?: string;
  targetVersion: string;
  upgradeSteps: string[];
  backupSteps: string[];
 ...
 */
export async function createTroubleshootingGuide(): Promise<{
  commonIssues: Array<{
    issue: string;
    symptoms: string[];
    solutions: string[];

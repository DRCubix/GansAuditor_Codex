...
export class FileSystemUtils {
   ...
   */
  async readFile(filePath: string): Promise<{ success: boolean; content?: string; error?: GanAuditorError }> {
  ...
  async writeFile(
    filePath: string, 
    content: string, 
    options: { createDirectories?: boolean; backup?: boolean } = {}
  ): Promise<{ success: boolean; error?: GanAuditorError }> {
   ...
   */
  async appendFile(filePath: string, content: string): Promise<{ success: boolean; error?: GanAuditorError }> {
   ...
   */
  async getStats(path: string): Promise<{ success: boolean; stats?: any; error?: GanAuditorError }> {
    try {
   ...
   */
  async ensureDirectory(dirPath: string): Promise<{ success: boolean; created?: boolean; error?: GanAuditorError }> {
  ...
  ): Promise<{ success: boolean; entries?: string[]; error?: GanAuditorError }> {
   ...
   */
  async delete(path: string, options: { recursive?: boolean } = {}): Promise<{ success: boolean; error?: GanAuditorError }> {
  ...
  async createTempFile(
    prefix: string = 'gan-auditor', 
    suffix: string = '.tmp'
  ): Promise<{ success: boolean; path?: string; error?: GanAuditorError }> {
   ...
   * Calculate file hash
   */
  async calculateHash(filePath: string, algorithm: string = 'sha256'): Promise<{ success: boolean; hash?: string; error?: GanAuditorError }> {
  ...
  ): Promise<{ valid: boolean; reason?: string; error?: GanAuditorError }> {
    try {
      ...
      for (const blockedPath of this.config.blockedPaths) {
        if (relativePath.startsWith(blockedPath)) {
          ...
          return { 
            valid: false, 
            reason: 'blocked-path',
            error: { name: 'FileSystemError', message: error.message } as unknown as GanAuditorError
          };
        }
      }
      ...
      return { 
        ...
        error: { name: 'FileSystemError', message: (error as Error).message } as unknown as GanAuditorError
      };
    }
  }
    ...
    const processDirectory = async (currentPath: string) => {
      ...
      for (const entry of dirEntries) {
        ...
        if (stats.isDirectory()) {
          ...
          const isBlocked = this.config.blockedPaths.some(blocked => 
            relativePath.includes(blocked)
          );
  ...
  private async createBackup(filePath: string): Promise<void> {
    ...
    logger.debug(
      ...
      { original: filePath, backup: backupPath },
      'file-utils'
    );
  }
}
 ...
 */
export function configureFileUtils(config: Partial<FileSystemConfig>): void {
  (fileUtils as any).config = { ...DEFAULT_FS_CONFIG, ...config };
}
 ...
 */
export async function readFileSafe(filePath: string): Promise<{ success: boolean; content?: string; error?: GanAuditorError }> {
 ...
 */
export async function ensureDirectorySafe(dirPath: string): Promise<{ success: boolean; created?: boolean; error?: GanAuditorError }> {

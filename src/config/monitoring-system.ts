...
export class SystemPromptMonitoringSystem extends EventEmitter {
   ...
   * Trigger an alert
   */
  private triggerAlert(alert: Alert, metricValue: number): void {
   ...
   * Get recent alerts
   */
  private getRecentAlerts(windowMs: number): AlertEvent[] {
    ...
    try {
      ...
      return lines
        .map(line => JSON.parse(line) as AlertEvent)
   ...
   * Write alert to file
   */
  private writeAlertToFile(alertEvent: AlertEvent): void {
    try {
      const alertsFile = join(this.config.outputDirectory, 'alerts.json');
   ...
   */
  private async sendWebhook(alertEvent: AlertEvent): Promise<void> {
    if (!this.config.webhookUrl) {
      return;
    }
 ...
 */
export function createMonitoringSystem(config?: Partial<MonitoringConfig>): SystemPromptMonitoringSystem {
 ...
 */
let globalMonitoringInstance: SystemPromptMonitoringSystem | null = null;

/**
 * Get or create global monitoring instance
 */
export function getGlobalMonitoring(config?: Partial<MonitoringConfig>): SystemPromptMonitoringSystem {
...
export const Metrics = {
  recordConfigValidation: (success: boolean, duration: number, errors?: string[]) => {
    getGlobalMonitoring().recordConfigValidation(success, duration, errors);
  },
  
  recordConfigDeployment: (environment: string, success: boolean, duration: number, backupCreated: boolean) => {
    getGlobalMonitoring().recordConfigDeployment(environment, success, duration, backupCreated);
  },
  
  recordMigration: (fromVersion: string, toVersion: string, success: boolean, duration: number) => {
    getGlobalMonitoring().recordMigration(fromVersion, toVersion, success, duration);
  },
  
  recordFeatureFlagEvaluation: (flagName: string, enabled: boolean, cacheHit: boolean) => {
    getGlobalMonitoring().recordFeatureFlagEvaluation(flagName, enabled, cacheHit);
  },
  
  recordAuditQuality: (success: boolean, score: number, completionTime: number, iterations: number) => {
 ...
 */
export function initializeMonitoringFromEnv(): SystemPromptMonitoringSystem {
  const config: Partial<MonitoringConfig> = {
    ...
    logLevel: (process.env.GAN_AUDITOR_LOG_LEVEL as any) || 'info',

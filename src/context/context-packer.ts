...
import type { IContextPacker, IGitHelper, IFileSystemHelper } from '../types/integration-types.js';
...
const FILE_RELEVANCE_WEIGHTS: Record<string, number> = {
  '.ts': 1.0,
  '.js': 1.0,
  '.tsx': 0.9,
  '.jsx': 0.9,
 ...
 */
class GitHelper implements IGitHelper {
  async getDiff(cwd: string = process.cwd()): Promise<string> {
    try {
  ...
  async getBranch(cwd: string = process.cwd()): Promise<string> {
    try {
  ...
  async getRepoRoot(cwd: string = process.cwd()): Promise<string> {
    try {
  ...
  async getFileTree(cwd: string = process.cwd(), maxDepth: number = MAX_TREE_DEPTH): Promise<string> {
    try {
  ...
  private async executeGitCommand(args: string[], cwd: string): Promise<{ stdout: string; stderr: string }> {
  ...
  private buildTreeStructure(files: string[], maxDepth: number): string {
  ...
  private async buildFileSystemTree(cwd: string, maxDepth: number): Promise<string> {
 ...
 */
function calculateFileRelevance(filePath: string, stats: { size: number }): number {
  let score = 0;
...
export class ContextPacker implements IContextPacker {
  private gitHelper: IGitHelper;
  private fsHelper: IFileSystemHelper;
   ...
   */
  async buildContextPack(config: SessionConfig, cwd: string = process.cwd()): Promise<string> {
    try {
      let context = '';
   ...
   */
  async buildDiffContext(cwd: string = process.cwd()): Promise<string> {
    try {
   ...
   */
  async buildPathsContext(paths: string[], cwd: string = process.cwd()): Promise<string> {
   ...
   */
  async buildWorkspaceContext(cwd: string = process.cwd()): Promise<string> {
    try {
   ...
   * Requirement 4.2: Git integration functions
   */
  private async getGitHeader(cwd: string): Promise<string> {
    try {
   ...
   * Requirement 4.3: File relevance scoring algorithm
   */
  private async pickRelevantFiles(root: string, limit: number): Promise<FileScore[]> {
   ...
   * Requirement 4.4: Context size management with truncation
   */
  private async collectTopSnippets(paths: string[], topK: number, maxTokens: number): Promise<string> {
    let context = '';
    let totalTokens = 0;
   ...
   */
  private createFileSnippet(filePath: string, content: string, maxLength: number): string | null {
    ...
    if (headerFooterLength >= maxLength) return null;
   ...
   */
  private async buildDirectoryContext(fullPath: string, relativePath: string, cwd: string): Promise<string> {
   ...
   */
  private async buildFileContext(fullPath: string, relativePath: string): Promise<string> {
    try {
   ...
   */
  private truncateContext(context: string, maxSize: number): string {

...
import { ProcessManager, type ProcessExecutionOptions, type ProcessResult } from './process-manager.js';
import { EnvironmentManager, type WorkingDirectoryResult, type EnvironmentResult } from './environment-manager.js';
import { CodexValidator, type CodexValidationResult } from './codex-validator.js';
import { errorDiagnosticSystem, type CodexDiagnostic, type ExecutionContext } from '../utils/error-diagnostic-system.js';
...
export interface CodexJudgeConfig {
  ...
  failFast: boolean; // No fallbacks allowed in production
}
...
export class CodexJudge implements ICodexJudge {
  ...
  private readonly codexValidator: CodexValidator;
  private readonly componentLogger = createComponentLogger('codex-judge');
  ...
  async executeAudit(request: AuditRequest): Promise<GanReview> {
    ...
    if (lastError) {
      ...
      const enhancedError = new Error(
        `Codex execution failed after ${this.config.retries + 1} attempts: ${lastError.message}`
      );
   ...
   */
  private async executeCodexCommandWithProcessManager(
    ...
    if (!codexPathResult.success || !codexPathResult.executablePath) {
      throw new CodexNotAvailableError(
        `Codex CLI executable not found: ${codexPathResult.error?.message || 'Unknown error'}`
      );
    }
    ...
    const result = await this.processManager.executeCommand(
      codexPathResult.executablePath,
      args,
      processOptions
    );
    ...
    if (result.exitCode !== 0) {
      const error = new Error(
        `Codex CLI failed with exit code ${result.exitCode}: ${result.stderr || 'No error details'}`
      );
      
      // Add process result details to error
      (error as any).processResult = {
...
Required JSON format:
{
   ...
   */
  private formatRubricDimensions(rubric: any): string {
    return rubric.dimensions
      .map((d: any) => `- ${d.name} (weight: ${d.weight}): ${d.description || 'Evaluate this aspect of the code'}`)
      .join('\n');
  }
   ...
   */
  private generateDimensionTemplate(rubric: any): string {
    return rubric.dimensions
      .map((d: any) => `{"name": "${d.name}", "score": <number>}`)
      .join(',\n    ');
  }
  ...
  private parseCodexResponse(rawResponse: string): GanReview {
    ...
    try {
      ...
      try {
        ...
        const error = new CodexResponseError(
          `Failed to parse Codex CLI response as JSON: ${jsonError instanceof Error ? jsonError.message : String(jsonError)}`,
          rawResponse
        );
   ...
   */
  private parseCodexJsonLines(rawResponse: string): CodexRawResponse {
    ...
    if (!foundAgentMessage || !agentMessage) {
      throw new CodexResponseError(
        'No agent message found in Codex JSONL response',
        rawResponse
      );
    }
   ...
   */
  private extractJsonFromNaturalLanguage(message: string): CodexRawResponse {
    // Try to extract JSON block from the message
    const jsonMatch = message.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      throw new CodexResponseError(
        'No JSON object found in Codex CLI response',
        message
      );
    }
    ...
    try {
      ...
      if (!this.isValidCodexResponse(extractedJson)) {
        throw new CodexResponseError(
          'Extracted JSON does not match expected Codex response format',
          message
        );
      }
   ...
   * Requirements: 1.2, 4.1 - Strict response validation without fallbacks
   */
  private isValidCodexResponse(response: any): boolean {
   ...
   */
  private validateScore(score: unknown, errors: string[]): number {
   ...
   */
  private validateVerdict(verdict: unknown, errors: string[]): GanVerdict {
  ...
  private validateDimensions(
    ...
    const validDimensions: DimensionalScore[] = [];
   ...
   */
  private validateReview(review: unknown, errors: string[]): ReviewDetails {
    ...
    if (r.inline !== undefined) {
      if (!Array.isArray(r.inline)) {
        ...
        result.inline = r.inline.filter((comment: unknown, index: number) => {
    ...
    if (r.citations !== undefined) {
      if (!Array.isArray(r.citations)) {
        ...
        result.citations = r.citations.filter((citation: unknown, index: number) => {
   ...
   */
  private validateIterations(iterations: unknown, errors: string[]): number {
  ...
  private isValidInlineComment(comment: unknown): comment is InlineComment {
    return (
      ...
      Number.isInteger((comment as any).line) &&
      (comment as any).line > 0 &&
   ...
   */
  private createDefaultDimensions(overallScore: number): DimensionalScore[] {
   ...
   */
  private createDefaultReview(): ReviewDetails {
    return {
   ...
   */
  private createDefaultJudgeCards(overallScore: number): JudgeCard[] {
    return [{
      model: 'codex-cli',
      score: overallScore,
   ...
   */
  private createEnhancedError(originalError: Error, diagnostic: CodexDiagnostic): Error {
    ...
    const enhancedError = new (originalError.constructor as any)(enhancedMessage);
    
    // Preserve original error properties
    Object.setPrototypeOf(enhancedError, Object.getPrototypeOf(originalError));
    
    // Add diagnostic information
    (enhancedError as any).diagnostic = diagnostic;
   ...
   */
  async executeAuditWithSystemPrompt(): Promise<never> {
    throw new Error(
      'executeAuditWithSystemPrompt is not supported in production mode. ' +
  ...
  parseWorkflowStepResults(): never {
    throw new Error(
      'parseWorkflowStepResults is not supported in production mode. ' +
  ...
  validatePromptDrivenResponse(): never {
    throw new Error(
      'validatePromptDrivenResponse is not supported in production mode. ' +
  ...
  generateStructuredAuditPrompt(): never {
    throw new Error(
      'generateStructuredAuditPrompt is not supported in production mode. ' +

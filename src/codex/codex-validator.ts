...
export class CodexValidationError extends Error {
  constructor(
    message: string,
    public readonly validationResult: CodexValidationResult
  ) {
    super(message);
...
export class CodexValidator {
   ...
   */
  async validateCodexAvailability(): Promise<CodexValidationResult> {
    const result: CodexValidationResult = {
      isAvailable: false,
      version: null,
      executablePath: null,
      environmentIssues: [],
    ...
    try {
      ...
      if (!versionInfo.isCompatible) {
        result.environmentIssues.push(
          `Codex CLI version ${versionInfo.version} is not compatible (minimum required: ${this.minVersion})`
        );
   ...
   */
  async resolveExecutablePath(): Promise<string | null> {
   ...
   */
  private async validateExecutablePermissions(path: string): Promise<boolean> {
    try {
   ...
   */
  async getVersionInfo(executablePath: string): Promise<CodexVersionInfo | null> {
    try {
   ...
   */
  private async testBasicFunctionality(executablePath: string): Promise<{ success: boolean; error?: string }> {
    try {
   ...
   * Validate search paths
   */
  private async validateSearchPaths(): Promise<{
    searchPaths: string[];
    foundPath: string | null;
    isExecutable: boolean;
  }> {
   ...
   */
  private generateInstallationGuidance(): string[] {

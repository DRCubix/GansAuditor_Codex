 ...
 */
export async function executeTestsStep(
  inputs: TestsStepInputs,
  ...
  try {
    ...
    // Set outputs
    const testsOutputs: TestsStepOutputs = {
      testResults,
      coverageGaps,
      suggestedTests,
      testQualityMetrics
 ...
 */
async function executeTestSuite(inputs: TestsStepInputs): Promise<TestResult[]> {
 ...
 */
async function detectTestFramework(workspacePath: string): Promise<string> {
  try {
 ...
 */
async function findTestFiles(workspacePath: string, testDirectories?: string[]): Promise<string[]> {
 ...
 */
async function scanForTestFiles(dirPath: string, relativePath: string, testFiles: string[]): Promise<void> {
  const { readdir, stat } = await import('fs/promises');
 ...
 */
async function simulateTestExecution(testFile: string, framework: string): Promise<TestResult[]> {
  const results: TestResult[] = [];
  ...
  const baseResult = {
    ...
    type: testFile.includes('integration') ? 'integration' as const : 'unit' as const
  };
 ...
 */
async function identifyCoverageGaps(inputs: TestsStepInputs): Promise<CoverageGap[]> {
 ...
 * Find source files that should be tested
 */
async function findSourceFiles(workspacePath: string, touchedFiles?: string[]): Promise<string[]> {
 ...
 */
async function analyzeCoverageForFile(sourceFile: string): Promise<CoverageGap> {
 ...
 */
async function createFocusedTestSuggestions(inputs: TestsStepInputs): Promise<string[]> {
  ...
  try {
    ...
    suggestions.push(
      ...
      'Add tests for async/await error handling',
 ...
 */
async function generateTestSuggestionsForFile(sourceFile: string): Promise<string[]> {
  const suggestions: string[] = [];
 ...
 */
async function validateTestQuality(inputs: TestsStepInputs): Promise<TestQualityMetrics> {
  try {
    // Find all test files
    const testFiles = await findTestFiles(inputs.workspacePath, inputs.testDirectories);
 ...
 * Add evidence based on test analysis results
 */
async function addTestAnalysisEvidence(
  outputs: TestsStepOutputs,
  evidence: EvidenceItem[]
): Promise<void> {
 ...
 */
export const DEFAULT_TESTS_INPUTS: Partial<TestsStepInputs> = {

 ...
 */
export async function executeReproStep(
  inputs: ReproStepInputs,
  ...
  try {
    ...
    // Set outputs
    const reproOutputs: ReproStepOutputs = {
      reproductionSteps,
      currentBehavior,
      expectedBehavior,
      minimalCommands,
 ...
 */
async function generateReproductionSteps(inputs: ReproStepInputs): Promise<string[]> {
  const steps: string[] = [];
 ...
 */
async function detectBuildStep(workspacePath: string): Promise<string | null> {
  try {
 ...
 */
async function detectTestStep(workspacePath: string): Promise<string | null> {
  try {
    const packageJsonPath = join(workspacePath, 'package.json');
    const packageContent = await readFile(packageJsonPath, 'utf-8');
    const packageJson = JSON.parse(packageContent);
 ...
 */
async function verifyCurrentBehavior(inputs: ReproStepInputs): Promise<string> {
  let behavior = "";
 ...
 */
function analyzeGitDiffBehavior(gitDiff: string): string {
  let analysis = "";
 ...
 */
function analyzeTouchedFilesBehavior(touchedFiles: string[]): string {
  let analysis = "";
  
  const sourceFiles = touchedFiles.filter(f => 
    !f.includes('test') && 
    !f.includes('spec') && 
    (f.endsWith('.ts') || f.endsWith('.js') || f.endsWith('.tsx') || f.endsWith('.jsx'))
  );
  
  const testFiles = touchedFiles.filter(f => 
    f.includes('test') || f.includes('spec')
  );
  
  const configFiles = touchedFiles.filter(f => 
 ...
 */
async function documentExpectedBehavior(inputs: ReproStepInputs): Promise<string> {
 ...
 */
async function createMinimalCommands(inputs: ReproStepInputs): Promise<string[]> {
 ...
 */
async function detectBuildCommand(workspacePath: string): Promise<string | null> {
  try {
    const packageJsonPath = join(workspacePath, 'package.json');
    const packageContent = await readFile(packageJsonPath, 'utf-8');
    const packageJson = JSON.parse(packageContent);
 ...
 */
async function detectLintCommand(workspacePath: string): Promise<string | null> {
  try {
    const packageJsonPath = join(workspacePath, 'package.json');
    const packageContent = await readFile(packageJsonPath, 'utf-8');
    const packageJson = JSON.parse(packageContent);
 ...
 */
async function generateVerificationCommands(inputs: ReproStepInputs): Promise<string[]> {
  const commands: string[] = [];
 ...
 */
async function detectTypeCheckCommand(workspacePath: string): Promise<string | null> {
  try {
    const packageJsonPath = join(workspacePath, 'package.json');
    const packageContent = await readFile(packageJsonPath, 'utf-8');
    const packageJson = JSON.parse(packageContent);
 ...
 */
async function validateReproductionQuality(
  outputs: ReproStepOutputs,
  evidence: EvidenceItem[]
): Promise<void> {
  ...
  const vagueSteps = outputs.reproductionSteps.filter(step => 
    step.length < 10 || 
    step.includes("TODO") || 
 ...
 */
export const DEFAULT_REPRO_INPUTS: Partial<ReproStepInputs> = {

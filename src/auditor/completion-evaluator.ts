...
export interface CompletionCriteria {
  tier1: { score: number; maxLoops: number }; // 95%@10
  tier2: { score: number; maxLoops: number }; // 90%@15
  tier3: { score: number; maxLoops: number }; // 85%@20
  hardStop: { maxLoops: number }; // 25 loops
  stagnationCheck: { startLoop: number; similarityThreshold: number };
}
...
export class CompletionEvaluator {
  ...
  constructor(criteria: CompletionCriteria = DEFAULT_COMPLETION_CRITERIA) {
   ...
   */
  private getApplicableThreshold(loop: number): number {
   ...
   */
  private getProgressMessage(score: number, loop: number): string {
    const threshold = this.getApplicableThreshold(loop);
   ...
   */
  private calculateFailureRate(sessionState: GansAuditorCodexSessionState): number {
    if (sessionState.history.length === 0) {
      return 0;
    }

    const failedAudits = sessionState.history.filter(
      entry => entry.review.verdict === "reject"
    ).length;
   ...
   */
  private extractCriticalIssues(sessionState: GansAuditorCodexSessionState): string[] {
    const issues: string[] = [];
...
export function createCompletionEvaluator(
  customCriteria?: Partial<CompletionCriteria>
): CompletionEvaluator {
 ...
 */
export function validateCompletionCriteria(criteria: CompletionCriteria): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];

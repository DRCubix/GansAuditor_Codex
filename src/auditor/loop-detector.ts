...
export class LoopDetector {
   ...
   */
  public analyzeResponseSimilarity(responses: string[]): SimilarityAnalysis {
   ...
   */
  public detectStagnation(iterations: IterationData[], currentLoop: number): DetailedStagnationAnalysis {
    ...
    const result: DetailedStagnationAnalysis = {
      isStagnant,
   ...
   */
  private structuralSimilarity(str1: string, str2: string): number {
   ...
   */
  private extractStructuralElements(code: string): string[] {
   ...
   */
  private detectContentPatterns(responses: string[]): string[] {
    const patterns: string[] = [];
    ...
    for (let i = 1; i < codeBlocks.length; i++) {
      const commonBlocks = codeBlocks[i].filter(block => 
        codeBlocks[i - 1].some(prevBlock => this.calculateStringSimilarity(block, prevBlock) > 0.9)
      );
   ...
   */
  private analyzeProgressPatterns(iterations: IterationData[]): DetailedStagnationAnalysis['progressAnalysis'] {
   ...
   */
  private areStuckOnSameIssues(reviews: any[]): boolean {
    ...
    const issueTypes = reviews.map(review => {
      ...
      if (review.inline) {
        for (const comment of review.inline) {
          ...
          if (comment.comment.toLowerCase().includes('security')) issues.push('security');
          if (comment.comment.toLowerCase().includes('performance')) issues.push('performance');
          if (comment.comment.toLowerCase().includes('style')) issues.push('style');
          if (comment.comment.toLowerCase().includes('logic')) issues.push('logic');
        }
      }
      return issues;
    });
    ...
    if (recentIssues.length < 2) return false;
   ...
   */
  private areCosmeticChangesOnly(codes: string[]): boolean {
   ...
   */
  private normalizeForCosmeticComparison(code: string): string {
    return code
   ...
   */
  private isRevertingChanges(codes: string[]): boolean {
   ...
   */
  private showsConfusion(codes: string[], scores: number[]): boolean {
   ...
   */
  private isProgressStagnant(scores: number[], similarities: number[]): boolean {
   ...
   */
  private detectDetailedPatterns(iterations: IterationData[]): DetailedStagnationAnalysis['patterns'] {
    const codes = iterations.map(iter => iter.code);
    const reviews = iterations.map(iter => iter.auditResult.review);
   ...
   */
  private categorizeError(comment: string): string | null {
   ...
   */
  private findPersistentIssues(reviews: any[]): string[] {
  ...
  private generateAlternativeSuggestions(
    progressAnalysis: DetailedStagnationAnalysis['progressAnalysis'],
    patterns: DetailedStagnationAnalysis['patterns'],
    scores: number[]
  ): string[] {
   ...
   */
  private createNoStagnationResult(currentLoop: number, reason: string): DetailedStagnationAnalysis {
    return {
      isStagnant: false,
      detectedAtLoop: currentLoop,
      similarityScore: 0,

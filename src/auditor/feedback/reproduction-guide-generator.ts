...
// ============================================================================
// Reproduction Guide Generator Configuration
// ============================================================================
...
export class ReproductionGuideGenerator {
  private config: ReproductionGuideConfig;
   ...
   */
  async generateReproductionGuide(context: ReproductionGuideContext): Promise<ReproductionGuide> {
    // Generate metadata
   ...
   */
  private async generateReproductionSteps(context: ReproductionGuideContext): Promise<ReproductionStep[]> {
   ...
   */
  private generateEnvironmentSetupSteps(context: ReproductionGuideContext): Omit<ReproductionStep, 'stepNumber'>[] {
    const steps: Omit<ReproductionStep, 'stepNumber'>[] = [];
   ...
   */
  private generateRepositorySetupSteps(context: ReproductionGuideContext): Omit<ReproductionStep, 'stepNumber'>[] {
    const steps: Omit<ReproductionStep, 'stepNumber'>[] = [];
   ...
   */
  private async generateIssueReproductionSteps(
    evidence: EvidenceItem,
    context: ReproductionGuideContext
  ): Promise<Omit<ReproductionStep, 'stepNumber'>[]> {
    const steps: Omit<ReproductionStep, 'stepNumber'>[] = [];
   ...
   */
  private async generateVerificationSteps(context: ReproductionGuideContext): Promise<VerificationStep[]> {
  ...
  private generateDiffVerificationSteps(
    diff: ProposedDiff,
    context: ReproductionGuideContext
  ): Omit<VerificationStep, 'stepNumber'>[] {
    const steps: Omit<VerificationStep, 'stepNumber'>[] = [];
   ...
   */
  private generateGeneralVerificationSteps(context: ReproductionGuideContext): Omit<VerificationStep, 'stepNumber'>[] {
    const steps: Omit<VerificationStep, 'stepNumber'>[] = [];
    const pm = context.repositoryInfo.packageManager;
   ...
   * Generate test commands
   */
  private generateTestCommands(context: ReproductionGuideContext): TestCommand[] {
   ...
   * Generate validation commands
   */
  private generateValidationCommands(context: ReproductionGuideContext): ValidationCommand[] {
   ...
   */
  private generateMetadata(context: ReproductionGuideContext): ReproGuideMetadata {
    const environmentRequirements: string[] = [];
  ...
  private generateTestFailureReproduction(
    evidence: EvidenceItem,
    context: ReproductionGuideContext
  ): Omit<ReproductionStep, 'stepNumber'>[] {
    const steps: Omit<ReproductionStep, 'stepNumber'>[] = [];
    const pm = context.repositoryInfo.packageManager;
  ...
  private extractFileFromLocation(location: string): string | null {
    const parts = location.split(":");
    return parts.length > 0 ? parts[0] : null;
  }

  private getLintConfigFile(projectConfig?: ProjectConfiguration): string | undefined {
  ...
  private getFormatConfigFile(projectConfig?: ProjectConfiguration): string | undefined {
  ...
  private generateTargetDescription(context: ReproductionGuideContext): string {
 ...
 */
export const DEFAULT_REPRODUCTION_GUIDE_CONFIG: ReproductionGuideConfig = {
 ...
 */
export function createReproductionGuideGenerator(
  config?: Partial<ReproductionGuideConfig>
 ...
 */
export function validateReproductionGuide(guide: ReproductionGuide): string[] {
  const errors: string[] = [];

...
export interface CodexHealthMonitorConfig {
  ...
  thresholds: {
    availabilityTimeout: number; // milliseconds
  ...
  alerting: {
    enabled: boolean;
    cooldownPeriod: number; // milliseconds between same alert types
 ...
 */
export const DEFAULT_CODEX_HEALTH_CONFIG: CodexHealthMonitorConfig = {
  enabled: true,
...
export class CodexHealthMonitor extends EventEmitter {
  private readonly config: CodexHealthMonitorConfig;
  ...
  private lastHealthReport?: CodexHealthReport;
  private activeAlerts: Map<string, CodexHealthAlert> = new Map();
  private alertCooldowns: Map<string, number> = new Map();

  // Health check history
  private healthHistory: CodexHealthCheckResult[] = [];
  ...
  async performHealthCheck(): Promise<CodexHealthReport> {
    ...
    try {
      ...
      this.componentLogger.error('Codex health check failed', error as Error);

      const failureReport: CodexHealthReport = {
        overall: 'critical',
        timestamp: Date.now(),
        uptime: Date.now() - this.startTime,
        checks: [{
   ...
   */
  private async checkCodexAvailability(): Promise<CodexHealthCheckResult> {
    ...
    try {
      const { stdout, stderr } = await execFileAsync(
        ...
        ['--help'],
        { timeout: this.config.thresholds.availabilityTimeout }
      );

      let status: CodexHealthStatus = 'healthy';
      ...
      let status: CodexHealthStatus = 'critical';
      let message = `Codex CLI unavailable: ${(error as Error).message}`;
   ...
   */
  private async checkCodexVersion(): Promise<CodexHealthCheckResult> {
    ...
    try {
      ...
      return {
        name: 'codex-version',
        status: 'warning',
        message: `Could not check Codex CLI version: ${(error as Error).message}`,
        timestamp: Date.now(),
        duration: 0,
        details: {
          error: (error as Error).message,
   ...
   */
  private async checkProcessManagerHealth(): Promise<CodexHealthCheckResult> {
    ...
    try {
      ...
      return {
        name: 'process-manager',
        status: 'critical',
        message: `Process manager health check failed: ${(error as Error).message}`,
        timestamp: Date.now(),
        duration: 0,
   ...
   */
  private async checkEnvironmentHealth(): Promise<CodexHealthCheckResult> {
    ...
    try {
      ...
      return {
        name: 'environment',
        status: 'critical',
        message: `Environment health check failed: ${(error as Error).message}`,
        timestamp: Date.now(),
        duration: 0,
   ...
   */
  private async checkPerformanceThresholds(): Promise<CodexHealthCheckResult> {
    ...
    try {
      ...
      return {
        name: 'performance-thresholds',
        status: 'critical',
        message: `Performance thresholds check failed: ${(error as Error).message}`,
        timestamp: Date.now(),
        duration: 0,
      };
    }
   ...
   */
  private async checkResourceUsage(): Promise<CodexHealthCheckResult> {
    ...
    try {
      ...
      return {
        name: 'resource-usage',
        status: 'critical',
        message: `Resource usage check failed: ${(error as Error).message}`,
        timestamp: Date.now(),
        duration: 0,
      };
    }
   ...
   */
  private async checkReliabilityMetrics(): Promise<CodexHealthCheckResult> {
    ...
    try {
      ...
      return {
        name: 'reliability-metrics',
        status: 'critical',
        message: `Reliability metrics check failed: ${(error as Error).message}`,
        timestamp: Date.now(),
        duration: 0,
      };
    }
   ...
   */
  private determineOverallHealth(checks: CodexHealthCheckResult[]): CodexHealthStatus {
  ...
  private processHealthAlerts(
    checks: CodexHealthCheckResult[],
    performance: CodexHealthReport['performance']
  ): CodexHealthAlert[] {
    if (!this.config.alerting.enabled) {
      return [];
    }

    const newAlerts: CodexHealthAlert[] = [];
   ...
   */
  private categorizeAlert(checkName: string): CodexHealthAlert['category'] {
   ...
   */
  private addToHealthHistory(checks: CodexHealthCheckResult[]): void {
   ...
   */
  private logHealthStatus(report: CodexHealthReport): void {
 ...
 */
export const startCodexHealthMonitoring = (config?: Partial<CodexHealthMonitorConfig>) => {
  if (config) {

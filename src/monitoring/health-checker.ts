...
export class HealthChecker extends EventEmitter {
  ...
  async performHealthCheck(): Promise<SystemHealthReport> {
    ...
    try {
      ...
      this.componentLogger.error('Health check failed', error as Error);
      
      const failureReport: SystemHealthReport = {
        overall: 'critical',
        timestamp: Date.now(),
        uptime: Date.now() - this.startTime,
        checks: [{
   ...
   */
  private async checkAuditEngine(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      return {
        name: 'audit-engine',
        status: 'critical',
        message: `Audit engine check failed: ${(error as Error).message}`,
        timestamp: Date.now(),
        duration: 0,
      };
    }
   ...
   */
  private async checkSessionManager(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      return {
        name: 'session-manager',
        status: 'critical',
        message: `Session manager check failed: ${(error as Error).message}`,
        timestamp: Date.now(),
        duration: 0,
      };
    }
   ...
   */
  private async checkCodexService(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      const { stdout, stderr } = await execFileAsync(
        ...
        ['--version'],
        { timeout: this.config.codexTimeout }
      );
      ...
      let message = `Codex service unavailable: ${(error as Error).message}`;
   ...
   */
  private async checkPerformanceThresholds(): Promise<HealthCheckResult> {
   ...
   */
  private determineOverallHealth(checks: HealthCheckResult[]): HealthStatus {
   ...
   */
  private generateRecommendations(checks: HealthCheckResult[], metrics: MetricsSnapshot): string[] {
 ...
 */
export function getCurrentHealthStatus(): HealthStatus {

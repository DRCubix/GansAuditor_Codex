...
import { healthChecker, type SystemHealthReport } from './health-checker.js';
import type {
...
export interface DebugConfig {
  ...
  performanceAnalysisWindow: number; // milliseconds
...
export class DebugTools {
   ...
   */
  async inspectSession(sessionId: string): Promise<SessionInspectionResult> {
    this.componentLogger.debug(`Inspecting session: ${sessionId}`);
    
    const result: SessionInspectionResult = {
      sessionId,
      exists: false,
      isValid: false,
      issues: [],
    ...
    try {
      ...
      try {
        ...
        const sessionState: GansAuditorCodexSessionState = JSON.parse(sessionData);
        ...
        result.issues.push(`Failed to parse session data: ${(error as Error).message}`);
        result.recommendations.push('Session file may be corrupted - consider deleting and recreating');
        return result;
      }

    } catch (error) {
      result.issues.push(`Session inspection failed: ${(error as Error).message}`);
  ...
  async listSessions(): Promise<string[]> {
    try {
      ...
      this.componentLogger.error('Failed to list sessions', error as Error);
      return [];
    }
  }
  ...
  async findSessions(criteria: {
    ...
    for (const sessionId of allSessions) {
      try {
        ...
        this.componentLogger.warn(`Failed to inspect session ${sessionId}`, { error: (error as Error).message });
      }
    }
   ...
   */
  async performSystemDiagnostics(): Promise<SystemDiagnosticResult> {
   ...
   */
  async analyzePerformance(timeWindow?: number): Promise<PerformanceAnalysisResult> {
    ...
    const resourceUsage = {
      ...
      memoryTrend: 'stable' as const, // Would need historical data to determine trend
    };
   ...
   */
  async exportDiagnostics(outputPath: string): Promise<void> {
   ...
   * Validate session state structure
   */
  private validateSessionState(state: GansAuditorCodexSessionState): string[] {
    const issues: string[] = [];
   ...
   * Generate recommendations based on session state
   */
  private generateSessionRecommendations(state: GansAuditorCodexSessionState): string[] {
    const recommendations: string[] = [];
   ...
   */
  private calculateImprovementTrend(scores: number[]): 'improving' | 'stagnant' | 'declining' {
    if (scores.length < 3) {
      return 'stagnant';
    }
   ...
   * Analyze session directory
   */
  private async analyzeSessionDirectory(): Promise<SystemDiagnosticResult['sessionDirectory']> {
    const result = {
   ...
   */
  private identifyBottlenecks(metrics: MetricsSnapshot): PerformanceAnalysisResult['bottlenecks'] {
   ...
   */
  private async getRecentSessionsSummary(count: number): Promise<Array<{
    sessionId: string;
    loops: number;
    isComplete: boolean;
    lastUpdated: number;
 ...
 */
export const inspectSession = (sessionId: string) => debugTools.inspectSession(sessionId);
export const analyzeSession = (sessionId: string) => debugTools.analyzeSession(sessionId);
export const listSessions = () => debugTools.listSessions();
export const findSessions = (criteria: Parameters<typeof debugTools.findSessions>[0]) => 
  debugTools.findSessions(criteria);
export const performSystemDiagnostics = () => debugTools.performSystemDiagnostics();
export const analyzePerformance = (timeWindow?: number) => debugTools.analyzePerformance(timeWindow);
export const exportDiagnostics = (outputPath: string) => debugTools.exportDiagnostics(outputPath);
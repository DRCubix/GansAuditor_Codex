 ...
 */
export class GanAuditorError extends Error {
    category;
    severity;
    recoverable;
    recovery_strategy;
    ...
    timestamp;
    component;
    constructor(message, category, severity = "medium", recoverable = true, recovery_strategy = "retry", suggestions = [], context = {}, component = "unknown") {
        super(message);
     ...
     */
    toStructuredResponse(fallback_data) {
        return {
            error: this.message,
            status: "failed",
            details: {
...
export class ConfigurationError extends GanAuditorError {
    constructor(message, suggestions = [], context = {}, component = "config-parser") {
        super(message, "config", "medium", true, "fallback", suggestions.length > 0 ? suggestions : [
...
export class InvalidConfigValueError extends ConfigurationError {
    constructor(field, value, validRange) {
        const suggestions = [
        ...
        super(`Invalid configuration value for '${field}': ${value}`, suggestions, { field, value, validRange }, "config-validator");
    }
}
/**
...
export class MissingConfigError extends ConfigurationError {
    constructor(field, scope) {
        ...
        super(`Missing required configuration: ${field}`, suggestions, { field, scope }, "config-validator");
    }
}
// ============================================================================
...
export class CodexError extends GanAuditorError {
    constructor(message, severity = "high", recovery_strategy = "fallback", suggestions = [], context = {}, component = "codex-judge") {
        super(message, "codex", severity, recovery_strategy !== "abort", recovery_strategy, suggestions.length > 0 ? suggestions : [
...
export class CodexNotAvailableError extends CodexError {
    constructor(executable = "codex") {
        super(`Codex CLI '${executable}' is not available`, "high", "fallback", [
...
export class CodexTimeoutError extends CodexError {
    constructor(timeout, command) {
        super(`Codex CLI execution timed out after ${timeout}ms`, "medium", "fallback", [
        ...
        ], { timeout, command }, "codex-execution");
    }
}
/**
...
export class CodexResponseError extends CodexError {
    constructor(message, rawResponse) {
        super(`Failed to parse Codex CLI response: ${message}`, "medium", "fallback", [
        ...
        ], { rawResponse: rawResponse?.substring(0, 500) }, "codex-parser");
    }
}
// ============================================================================
// File System Errors
...
export class FileSystemError extends GanAuditorError {
    constructor(message, severity = "medium", recovery_strategy = "skip", suggestions = [], context = {}, component = "filesystem") {
        super(message, "filesystem", severity, recovery_strategy !== "abort", recovery_strategy, suggestions.length > 0 ? suggestions : [
...
export class FileNotFoundError extends FileSystemError {
    constructor(path) {
        super(`File not found: ${path}`, "low", "skip", [
...
export class FileAccessError extends FileSystemError {
    constructor(path, operation) {
        super(`Access denied for ${operation} operation on: ${path}`, "medium", "skip", [
            "Check file and directory permissions",
            "Ensure the process has necessary access rights",
            "The system will skip this file and continue",
        ], { path, operation }, "file-access");
    }
}
/**
...
export class DirectoryCreationError extends FileSystemError {
    constructor(path, reason) {
        super(`Failed to create directory: ${path}${reason ? ` (${reason})` : ""}`, "medium", "retry", [
            "Check parent directory permissions",
            "Ensure sufficient disk space",
            "Verify the path is valid for the operating system",
        ], { path, reason }, "directory-manager");
    }
}
// ============================================================================
...
export class SessionError extends GanAuditorError {
    constructor(message, severity = "medium", recovery_strategy = "fallback", suggestions = [], context = {}, component = "session-manager") {
        super(message, "session", severity, recovery_strategy !== "abort", recovery_strategy, suggestions.length > 0 ? suggestions : [
...
export class SessionNotFoundError extends SessionError {
    constructor(sessionId) {
        super(`Session not found: ${sessionId}`, "low", "fallback", [
...
export class SessionPersistenceError extends SessionError {
    constructor(sessionId, operation, reason) {
        super(`Failed to ${operation} session ${sessionId}${reason ? `: ${reason}` : ""}`, "medium", "retry", [
            "Check session state directory permissions",
            "Ensure sufficient disk space",
            "The session will continue in memory only",
        ], { sessionId, operation, reason }, "session-persister");
    }
}
// ============================================================================
...
export class AuditServiceUnavailableError extends CodexError {
    constructor(service = "GAN Auditor", details) {
        super(`Audit service '${service}' is currently unavailable${details ? `: ${details}` : ""}`, "high", "fallback", [
        ...
        ], { service, details, timestamp: Date.now() }, "audit-service");
    }
}
/**
...
export class InvalidCodeFormatError extends GanAuditorError {
    constructor(format, expectedFormats = [], codeSnippet) {
        ...
        super(`Invalid code format detected: ${format}`, "config", "medium", true, "user_intervention", [
            formatGuidance,
...
export class SessionCorruptionError extends SessionError {
    constructor(sessionId, corruptionType, recoveryOptions = []) {
        ...
        super(`Session ${sessionId} is corrupted: ${corruptionType}`, "medium", "fallback", [
...
export class AuditTimeoutError extends CodexError {
    constructor(timeoutMs, partialResults, completionPercentage, operation = "audit") {
        const completionInfo = completionPercentage
            ? ` (${Math.round(completionPercentage)}% completed)`
            : "";
        super(`${operation} operation timed out after ${timeoutMs}ms${completionInfo}`, "medium", "fallback", [
...
export class ErrorClassifier {
    ...
    static classify(error) {
        ...
        return new FileSystemError(message, "medium", "retry", ["Check the error details and try again", "Contact support if the issue persists"], { originalError: message, stack }, "unknown");
    }
    static isConfigError(message) {
        const patterns = [
            /config/i,
            /validation/i,
            /invalid.*value/i,
    ...
    static isFileSystemError(message) {
        const patterns = [
            /ENOENT/i,
            /EACCES/i,
            /EPERM/i,
    ...
    static isWorkflowError(message) {
        const patterns = [
            /workflow/i,
            /step/i,
    ...
    static isAnalysisError(message) {
        const patterns = [
            /analysis/i,
            /completion/i,
            /evaluation/i,
...
export class ErrorRecovery {
     ...
     */
    static createFallbackData(error) {
        switch (error.category) {
            case "config":
...
export class PromptRenderingError extends GanAuditorError {
    constructor(message, templatePath, variables) {
        super(message, 'config', 'high', true, 'fallback', [
        ...
        ], { templatePath, variables: variables ? Object.keys(variables) : [] }, 'system-prompt-manager');
...
export class WorkflowStepError extends GanAuditorError {
    constructor(message, stepName, stepIndex, canSkip = false) {
        super(message, 'config', canSkip ? 'minor' : 'major', true, canSkip ? 'skip' : 'retry', [
        ...
        ], { stepName, stepIndex, canSkip }, 'audit-workflow');
...
export class CompletionAnalysisError extends GanAuditorError {
    constructor(message, currentLoop, scoreHistory) {
        super(message, 'config', 'minor', true, 'fallback', [
        ...
        ], { currentLoop, scoreHistoryLength: scoreHistory?.length || 0 }, 'completion-evaluator');
...
export class PromptContextError extends GanAuditorError {
    constructor(message, operation, sessionId) {
        super(message, 'session', operation === 'retrieve' ? 'low' : 'high', true, operation === 'store' ? 'skip' : 'fallback', [
            `Operation: ${operation}`,
        ...
        ], { operation, sessionId }, 'session-manager');
...
export class QualityProgressionError extends GanAuditorError {
    constructor(message, sessionId, thoughtNumber) {
        super(message, 'session', 'low', true, 'skip', [
        ...
        ], { sessionId, thoughtNumber }, 'session-manager');

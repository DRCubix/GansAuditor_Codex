 ...
 */
import { IssueCategory, Priority, Severity, CriticalIssueType, ProgressTrend, FeedbackDetailLevel, DEFAULT_ENHANCED_RESPONSE_CONFIG, } from './synchronous-response-types.js';
...
export class EnhancedResponseBuilder {
     ...
     */
    buildSynchronousResponse(standardResponse, auditResult, completionResult, sessionState, stagnationAnalysis, terminationResult) {
        ...
        if (auditResult && this.config.includeDetailedFeedback) {
            response.feedback = this.buildStructuredFeedback(auditResult, sessionState, stagnationAnalysis, terminationResult);
        }
        // Build completion status
        if (completionResult && auditResult) {
            response.completionStatus = this.buildCompletionStatus(completionResult, auditResult.overall, sessionState?.currentLoop || 1);
        }
        // Build loop information
        if (sessionState && this.config.includeLoopInfo) {
            response.loopInfo = this.buildLoopInfo(sessionState, stagnationAnalysis);
        }
        // Build termination information
        if (terminationResult?.shouldTerminate) {
            response.terminationInfo = this.buildTerminationInfo(terminationResult, sessionState);
        }
     ...
     */
    buildStructuredFeedback(auditResult, sessionState, stagnationAnalysis, terminationResult) {
        ...
        // Generate next steps
        const nextSteps = this.generateNextSteps(auditResult, improvements, criticalIssues, stagnationAnalysis);
        // Assess progress
        const progressAssessment = this.assessProgress(auditResult, sessionState, stagnationAnalysis);
        // Build summary
        const summary = this.buildFeedbackSummary(auditResult, improvements.length, criticalIssues.length, progressAssessment);
        return {
            summary,
     ...
     */
    extractImprovementSuggestions(auditResult) {
        const suggestions = [];
     ...
     */
    convertCommentToSuggestion(comment) {
     ...
     * Extract actionable guidance from comment
     */
    extractActionFromComment(comment) {
     ...
     */
    createDimensionalSuggestion(dimension) {
     ...
     */
    extractCriticalIssues(auditResult, terminationResult) {
        const issues = [];
     ...
     */
    convertCommentToCriticalIssue(comment) {
        if (!comment.comment) {
            return null;
        }
     ...
     */
    determineCriticalIssueType(comment) {
     ...
     */
    generateImpactDescription(type, severity) {
        const impactMap = {
     ...
     */
    generateResolutionGuidance(type, comment) {
        const resolutionMap = {
     ...
     */
    generateNextSteps(auditResult, improvements, criticalIssues, stagnationAnalysis) {
        const steps = [];
        let stepNumber = 1;
        ...
        const blockerIssues = criticalIssues.filter(issue => issue.severity === Severity.BLOCKER || issue.severity === Severity.CRITICAL);
        ...
        const highPriorityImprovements = improvements.filter(imp => imp.priority === Priority.CRITICAL || imp.priority === Priority.HIGH);
     ...
     */
    assessProgress(auditResult, sessionState, stagnationAnalysis) {
        let trend = ProgressTrend.IMPROVING;
        let confidence = 0.5;
        const factors = [];
     ...
     */
    buildFeedbackSummary(auditResult, improvementCount, criticalCount, progressAssessment) {
     ...
     */
    buildCompletionStatus(completionResult, score, currentLoop) {
     ...
     * Build loop information
     */
    buildLoopInfo(sessionState, stagnationAnalysis) {
        ...
        let progressTrend = ProgressTrend.IMPROVING;
        if (stagnationAnalysis?.isStagnant) {
     ...
     * Build termination information
     */
    buildTerminationInfo(terminationResult, sessionState) {
     ...
     * Build session metadata
     */
    buildSessionMetadata(sessionState) {
 ...
 */
export function createEnhancedResponseBuilder(config) {
    return new EnhancedResponseBuilder(config);
}
/**
 * Create a minimal enhanced response builder for basic feedback
 */
export function createMinimalEnhancedResponseBuilder() {
 ...
 */
export function createComprehensiveEnhancedResponseBuilder() {
    return new EnhancedResponseBuilder({
        includeDetailedFeedback: true,
...
//# sourceMappingURL=enhanced-response-builder.js.map
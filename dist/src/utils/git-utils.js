...
export class GitUtils {
    ...
    async executeCommand(args, options = {}) {
        ...
        try {
            ...
            const duration = Date.now() - startTime;
            logger.debug(`Git command failed: ${command}`, { error: error.message, cwd, duration }, 'git-utils');
            return {
                success: false,
                stdout: error.stdout || '',
                stderr: error.stderr || '',
     ...
     */
    async getLastCommitInfo(workingDir) {
        try {
     ...
     */
    createTreeStructure(files, maxDepth) {
        const tree = {};
 ...
 */
export function configureGitUtils(config) {
    gitUtils.config = { ...DEFAULT_GIT_CONFIG, ...config };
}
// ============================================================================
 ...
 */
export async function getGitRepoInfo(path) {

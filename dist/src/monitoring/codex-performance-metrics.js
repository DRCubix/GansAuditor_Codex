...
export class CodexPerformanceMetrics extends EventEmitter {
     ...
     */
    recordExecution(operationType, duration, success, timedOut = false, retryCount = 0, error) {
        if (!this.config.enabled)
            return;
    ...
    updateProcessState(activeProcesses, queueSize) {
        ...
        this.currentMetrics.peakConcurrency = Math.max(this.currentMetrics.peakConcurrency, activeProcesses);
    ...
    recordResourceSample() {
        ...
        // Update peaks
        this.currentMetrics.peakMemoryUsage = Math.max(this.currentMetrics.peakMemoryUsage, memoryUsage);
        this.currentMetrics.peakCpuUsage = Math.max(this.currentMetrics.peakCpuUsage, cpuUsage);
        this.cleanupOldSamples();
    }
    /**
     * Record throughput sample
     */
    recordThroughputSample(concurrentProcesses) {
        const timestamp = Date.now();
        const windowDuration = 60000; // 1 minute window
        const windowStart = timestamp - windowDuration;
        // Count operations in the last minute
        const recentExecutions = this.executionSamples.filter(sample => sample.timestamp >= windowStart);
        const sample = {
            timestamp,
 ...
 */
export const recordCodexExecution = (operationType, duration, success, timedOut, retryCount, error) => codexPerformanceMetrics.recordExecution(operationType, duration, success, timedOut, retryCount, error);
export const updateCodexProcessState = (activeProcesses, queueSize) => codexPerformanceMetrics.updateProcessState(activeProcesses, queueSize);
export const getCodexPerformanceSnapshot = () => codexPerformanceMetrics.getPerformanceSnapshot();
export const getCodexExecutionStatistics = () => codexPerformanceMetrics.getExecutionStatistics();
export const getCodexResourceStatistics = () => codexPerformanceMetrics.getResourceStatistics();
export const getCodexReliabilityStatistics = () => codexPerformanceMetrics.getReliabilityStatistics();
export const getCodexThroughputStatistics = () => codexPerformanceMetrics.getThroughputStatistics();
//# sourceMappingURL=codex-performance-metrics.js.map
...
export class CodexMonitoringSystem {
    ...
    start(components) {
        ...
        try {
            ...
            this.componentLogger.error('Failed to start Codex monitoring system', error);
            throw error;
        }
    }
    /**
    ...
    async stop() {
        ...
        try {
            ...
            this.componentLogger.error('Failed to stop Codex monitoring system', error);
            throw error;
        }
    }
    /**
    ...
    async getMonitoringStatus() {
        ...
        return {
            system: {
                ...
                uptime: Date.now() - this.componentLogger.startTime || 0,
                components: {
                    operationLogging: this.config.operationLogging.enabled,
                    performanceMetrics: this.config.performanceMetrics.enabled,
                    healthMonitoring: this.config.healthMonitoring.enabled,
    ...
    async exportMonitoringData(outputPath) {
        ...
        try {
            ...
            this.componentLogger.error('Failed to export monitoring data', error);
            throw error;
        }
    }
    /**
     ...
     */
    async executeMonitoredCommand(executable, args, options, operationType, sessionId, thoughtNumber) {
        // Start operation logging
        ...
        try {
            ...
            recordCodexExecution(operationType, result.executionTime, result.exitCode === 0 && !result.timedOut, result.timedOut, 0, // retry count
            result.exitCode !== 0 ? {
                category: 'execution',
                type: operationType,
            ...
            operationTimer.completeWithError(error);
            // Record performance metrics for failure
            recordCodexExecution(operationType, Date.now() - Date.now(), // This would be actual duration
            false, false, 0, {
                category: 'error',
                type: operationType,
            ...
            logCodexError(operationType, error, {
                executionId,
                sessionId,
                thoughtNumber,
            ...
            this.componentLogger.error('Monitored command execution failed', error, {
                executionId,
                operationType,
                sessionId,
                thoughtNumber,
     ...
     */
    setupPerformanceMetricsIntegration() {
 ...
 */
export function startCodexMonitoring(config, components) {
    if (config) {
 ...
 */
export async function stopCodexMonitoring() {
    await codexMonitoringSystem.stop();
}
/**
 * Get comprehensive monitoring status
 */
export async function getCodexMonitoringStatus() {
    return await codexMonitoringSystem.getMonitoringStatus();
}
/**
 * Get monitoring summary
 */
export function getCodexMonitoringSummary() {
    return codexMonitoringSystem.getMonitoringSummary();
}
/**
 * Execute a monitored Codex command
 */
export async function executeMonitoredCodexCommand(executable, args, options, operationType, sessionId, thoughtNumber) {
    return await codexMonitoringSystem.executeMonitoredCommand(executable, args, options, operationType, sessionId, thoughtNumber);
}
// ============================================================================
// Usage Example
// ============================================================================
...
export async function exampleUsage() {
    ...
    try {
        ...
        const result = await executeMonitoredCodexCommand('codex', ['exec', '--json', 'analyze this code'], {
            workingDirectory: process.cwd(),
            timeout: 30000,
            environment: process.env,
        }, 'audit_execution', 'example-session-123', 1);
...
//# sourceMappingURL=codex-monitoring-integration.js.map
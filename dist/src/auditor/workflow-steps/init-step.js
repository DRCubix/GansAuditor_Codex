 ...
 */
export async function executeInitStep(inputs, outputs, evidence) {
    try {
 ...
 * Extract task goals from Spec documents
 */
async function extractTaskGoals(specPath = '.kiro/specs', workspacePath) {
    const goals = [];
    try {
 ...
 */
async function findSpecDirectories(specPath, workspacePath) {
    const specDirs = [];
    try {
 ...
 */
function parseTaskGoalsFromRequirements(content) {
    const goals = [];
 ...
 */
async function parseAcceptanceCriteria(specPath = '.kiro/specs', workspacePath) {
    const criteria = [];
    try {
        const specDirs = await findSpecDirectories(specPath, workspacePath);
        for (const specDir of specDirs) {
 ...
 */
function parseAcceptanceCriteriaFromContent(content) {
    const criteria = [];
 ...
 * Identify constraints from Steering documents
 */
async function identifyConstraints(steeringPath = '.kiro/steering', workspacePath) {
    const constraints = [];
    try {
 ...
 */
function parseConstraintsFromContent(content, filename) {
    const constraints = [];
 ...
 */
async function detectTouchedFiles(workspacePath, gitDiff) {
    const touchedFiles = [];
    if (gitDiff) {
 ...
 */
function parseGitDiffFiles(gitDiff) {
    const files = [];
 ...
 */
async function findSourceFiles(workspacePath) {
    const sourceFiles = [];
    try {
        const { readdir, stat } = await import('fs/promises');
 ...
 */
async function validateSpecificationCompleteness(taskGoals, acceptanceCriteria, constraints, evidence) {
    ...
    const vagueACs = acceptanceCriteria.filter(ac => ac.description.length < 30 ||

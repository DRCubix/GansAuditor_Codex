 ...
 */
export async function executeConformStep(inputs, outputs, evidence) {
    try {
 ...
 */
async function validateNamingConventions(inputs) {
    const violations = [];
    try {
 ...
 * Load naming rules from Steering documents
 */
async function loadNamingRules(steeringPath, workspacePath) {
    const rules = {
 ...
 */
function parseNamingRulesFromContent(content) {
    const rules = {};
 ...
 */
async function analyzeFileNaming(file, workspacePath, rules) {
    const violations = [];
    try {
 ...
 */
function checkFileNaming(fileName, convention) {
        ...
        case 'kebab-case':
            if (!/^[a-z0-9]+(-[a-z0-9]+)*$/.test(baseName)) {
                return {
                    ...
                    suggested: baseName.replace(/[A-Z]/g, (match, offset) => (offset > 0 ? '-' : '') + match.toLowerCase()).replace(/[_\s]/g, '-')
                };
            }
            break;
        case 'camelCase':
 ...
 * Analyze code content for naming violations
 */
function analyzeCodeNaming(content, file, rules) {
    const violations = [];
 ...
 */
function isValidNaming(name, convention) {
    switch (convention) {
        case 'camelCase':
 ...
 */
function convertNaming(name, convention) {
    switch (convention) {
        case 'camelCase':
        ...
        case 'kebab-case':
            return name.replace(/[A-Z]/g, (match, offset) => (offset > 0 ? '-' : '') + match.toLowerCase()).replace(/_/g, '-');
        case 'snake_case':
            return name.replace(/[A-Z]/g, (match, offset) => (offset > 0 ? '_' : '') + match.toLowerCase()).replace(/-/g, '_');
        case 'SCREAMING_SNAKE_CASE':
 ...
 * Check architecture pattern compliance
 */
async function checkArchitectureCompliance(inputs) {
    const violations = [];
    try {
 ...
 */
function extractArchitecturePatterns(constraints) {
    const patterns = [];
 ...
 */
async function analyzeArchitectureCompliance(file, workspacePath, patterns) {
    const violations = [];
    try {
        const filePath = join(workspacePath, file);
        const content = await readFile(filePath, 'utf-8');
 ...
 * Validate library usage
 */
async function validateLibraryUsage(inputs) {
    const issues = [];
    try {
 ...
 * Analyze dependencies
 */
async function analyzeDependencies(inputs) {
    try {
        const packageJsonPath = join(inputs.workspacePath, 'package.json');
        const packageContent = await readFile(packageJsonPath, 'utf-8');
        const packageJson = JSON.parse(packageContent);
        ...
        if (totalDependencies > 100)
            bundleSizeImpact = 'Large';
        if (totalDependencies > 200)
            bundleSizeImpact = 'Very Large';
        const licenseIssues = [];
 ...
 * Add evidence based on conformance analysis results
 */
async function addConformanceEvidence(outputs, evidence) {
 ...
 */
export const DEFAULT_CONFORM_INPUTS = {
    workspacePath: process.cwd(),
    steeringPath: '.kiro/steering'
};
//# sourceMappingURL=conform-step.js.map
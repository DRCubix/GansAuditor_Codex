...
import { withRetry, handleConfigError, } from '../utils/error-handler.js';
...
export class GanAuditor {
    ...
    async auditThought(thought, sessionId) {
        ...
        try {
            ...
            const mergedConfig = await this.mergeInlineConfigWithErrorHandling(thought.thought, session.config);
            ...
            this.componentLogger.error('Audit failed - no fallback available', error, {
            ...
            if (error instanceof Error) {
                ...
                if (sessionId) {
                    try {
                        ...
                        this.componentLogger.warn('Could not record Codex failure in session', sessionError);
                    }
                }
            }
    ...
    async executeAudit(request) {
        try {
            ...
            this.componentLogger.error('Codex audit execution failed - no fallback available', error, {
                task: request.task,

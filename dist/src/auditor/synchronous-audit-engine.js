 ...
 */
export const DEFAULT_SYNCHRONOUS_AUDIT_ENGINE_CONFIG = {
...
export class SynchronousAuditEngine {
     ...
     * Requirements 9.1-9.4: Performance optimizations
     */
    async auditAndWait(thought, sessionId) {
        ...
        try {
            ...
            if (!formatValidation.isValid) {
                ...
                const formatError = new InvalidCodeFormatError(formatValidation.detectedFormat || 'unknown', ['markdown', 'typescript', 'javascript', 'python'], thought.thought.substring(0, 200));
                const formatResult = await handleInvalidCodeFormat(formatError, thought.thought, ['markdown', 'typescript', 'javascript', 'python']);
    ...
    initializePerformanceComponents() {
        ...
        if (perfConfig?.enableCaching) {
            this.auditCache = new AuditCache(perfConfig.cacheConfig);
        ...
        if (perfConfig?.enableProgressTracking) {
            this.progressTracker = new ProgressTracker(perfConfig.progressConfig);
        ...
        if (perfConfig?.enableQueueManagement) {
            this.auditQueue = new AuditQueue((thought, sessionId) => this.executeDirectAudit(thought, sessionId), perfConfig.queueConfig);
     ...
     */
    async executeOptimizedAudit(thought, sessionId) {
     ...
     */
    async executeDirectAudit(thought, sessionId) {
        return await withTimeout(() => this.ganAuditor.auditThought(thought, sessionId), this.config.auditTimeout, 'Audit operation timed out');
    }
    /**
     ...
     */
    createDisabledResult(thought, sessionId) {
        return {
     ...
     */
    createSkippedResult(thought, sessionId) {
        return {
     ...
     * Validate code format before processing (Requirement 7.2)
     */
    async validateCodeFormat(thought) {
        const issues = [];
     ...
     */
    async executeAuditWithRecovery(thought, sessionId) {
        ...
        try {
            ...
            try {
                return await withTimeout(() => this.ganAuditor.auditThought(thought, sessionId), reducedTimeout, 'Audit retry timed out');
            }
            catch (retryError) {
     ...
     */
    async handleTimeoutError(error, thought, sessionId, duration, partialResults, completionPercentage) {
        this.componentLogger.error('Audit operation timed out', error, {
            thoughtNumber: thought.thoughtNumber,
            sessionId,
            duration,
    ...
    destroy() {
        ...
        if (this.ganAuditor && typeof this.ganAuditor.destroy === 'function') {
            this.ganAuditor.destroy();
        }
    }
}
// ============================================================================
 ...
 */
export function createSynchronousAuditEngine(config = {}) {
    return new SynchronousAuditEngine(config);
}
/**
 * Create a Synchronous Audit Engine instance with custom GAN auditor
 */
export function createSynchronousAuditEngineWithAuditor(config, ganAuditor) {
    return new SynchronousAuditEngine(config, ganAuditor);
}
//# sourceMappingURL=synchronous-audit-engine.js.map
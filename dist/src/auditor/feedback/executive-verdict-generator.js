...
export class ExecutiveVerdictGenerator {
     ...
     */
    makeShipDecision(qualityAssessment, evidence) {
        ...
        const failedRequiredDimensions = qualityAssessment.dimensionEvaluations.filter(dim => dim.dimension.required && !dim.passed);
     ...
     */
    generateSummary(qualityAssessment, evidence, decision) {
        const summary = [];
     ...
     */
    generateNextSteps(qualityAssessment, evidence, decision, iteration) {
        const nextSteps = [];
        if (decision === "no-ship") {
     ...
     */
    generateJustification(qualityAssessment, evidence, decision) {
     ...
     */
    generatePrimaryReasons(qualityAssessment, evidence, decision) {
        const reasons = [];
        if (decision === "ship") {
     ...
     */
    generateRiskAssessment(qualityAssessment, evidence) {
     ...
     */
    identifyRisks(qualityAssessment, evidence) {
        const risks = [];
        // Security risks
        ...
        // Test coverage risks
        const testingDim = qualityAssessment.dimensionEvaluations.find(dim => dim.dimension.id === "testing_quality");
        ...
        const styleConventionsDim = qualityAssessment.dimensionEvaluations.find(dim => dim.dimension.id === "style_conventions");
     ...
     */
    assessOverallRisk(qualityAssessment, evidence) {
        const criticalIssues = this.getCriticalIssues(qualityAssessment, evidence);
        if (criticalIssues.length > 0) {
            return "critical";
        }
     ...
     */
    calculateConfidenceLevel(qualityAssessment, evidence) {
        let confidence = 100;
     ...
     */
    getCriticalIssues(qualityAssessment, evidence) {
        const issues = [];
     ...
     */
    isRiskAboveTolerance(riskLevel) {
        const riskLevels = {
            "low": 1,
            "medium": 2,
            "high": 3,
 ...
 */
export const DEFAULT_EXECUTIVE_VERDICT_CONFIG = {
 ...
 */
export function createExecutiveVerdictGenerator(config) {
    return new ExecutiveVerdictGenerator(config);
}
/**
 * Validate executive verdict structure
 */
export function validateExecutiveVerdict(verdict) {
    const errors = [];
...
//# sourceMappingURL=executive-verdict-generator.js.map
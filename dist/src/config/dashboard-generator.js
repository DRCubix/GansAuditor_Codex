...
export class DashboardGenerator {
    config;
    monitoring;
    constructor(monitoring, config = DEFAULT_DASHBOARD_CONFIG) {
...
<body class="${this.config.theme}">
    ...
    generateMetricsReport() {
        ...
        for (const metric of trendMetrics) {
            const current = dashboardData.metrics[metric] || 0;
     ...
     */
    generateRecentActivitySection(data) {
        return `
        <section class="dashboard-section">
     ...
     */
    generateFeatureFlagsSection(data) {
    ...
    getChartData(chart, data) {
        const metricValue = data.metrics[chart.metric] || 0;
        if (chart.type === 'gauge') {
 ...
 */
export function createDashboardGenerator(monitoring, config) {
 ...
 */
export function generateDashboard(monitoring, outputPath) {

 ...
 */
import { DEFAULT_GANSAUDITOR_CODEX_SESSION_CONFIG, CONFIG_CONSTRAINTS, DEFAULT_SESSION_CONFIG, } from '../types/gan-types.js';
// ============================================================================
 ...
 */
export function extractInlineConfig(thought) {
    ...
    const parseResult = parseJsonWithFallback(configText);
 ...
 */
export function parseJsonWithFallback(jsonText) {
    try {
        ...
        const greedyResult = attemptGreedyJsonParse(jsonText);
 ...
 */
function attemptGreedyJsonParse(text) {
    // Try to find JSON object boundaries
    const objectMatches = text.match(/\{[\s\S]*\}/);
    if (objectMatches) {
        try {
            const data = JSON.parse(objectMatches[0]);
            return { success: true, data };
        }
        catch {
    ...
    if (arrayMatches) {
        try {
            const data = JSON.parse(arrayMatches[0]);
            return { success: true, data };
        }
        catch {
            // Continue to failure
 ...
 */
export function validateAndSanitizeConfig(inlineConfig = {}, baseConfig = DEFAULT_GANSAUDITOR_CODEX_SESSION_CONFIG) {
    const errors = [];
    const warnings = [];
 ...
 */
function sanitizeMaxCycles(value) {
    const changes = [];
    if (typeof value !== 'number' || isNaN(value) || !isFinite(value)) {
        return {
 ...
 */
export function mergeConfigurations(baseConfig, inlineConfig) {

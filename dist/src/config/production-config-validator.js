 ...
 */
import { parseProductionCodexConfigFromEnv, validateProductionCodexConfig } from './production-codex-config.js';
 ...
 */
export async function validateProductionConfiguration(options = { strict: false, skipCodexValidation: false }) {
    const errors = [];
    const warnings = [];
    let config;
    try {
 ...
 * Validate environment-specific requirements
 */
function validateEnvironmentRequirements(errors, warnings) {
 ...
 */
function validateMockFunctionalityDisabled(errors, warnings) {
 ...
 */
function validateRequiredEnvironmentVariables(errors, warnings) {
    const requiredVars = [
        'ENABLE_GAN_AUDITING',
        'CODEX_EXECUTABLE',
 ...
 */
async function validateCodexAvailability(config) {
 ...
 */
export function generateProductionReadinessReport(result) {
    const lines = [];
 ...
 */
export async function ensureProductionReady(options) {

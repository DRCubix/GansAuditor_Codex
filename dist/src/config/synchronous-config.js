 ...
 */
export const DEFAULT_AUDIT_TIMEOUT_CONFIG = {
 ...
 */
export const DEFAULT_SYNCHRONOUS_CONFIG = {
    enabled: false,
 ...
 */
function parseEnvBoolean(envVar, defaultValue) {
    if (!envVar)
        return defaultValue;
 ...
 */
function parseEnvInteger(envVar, defaultValue, min, max) {
    if (!envVar)
        return defaultValue;
 ...
 */
function parseEnvFloat(envVar, defaultValue, min, max) {
    if (!envVar)
        return defaultValue;
 ...
 */
function parseEnvString(envVar, defaultValue, validValues) {
    if (!envVar)
        return defaultValue;
    if (validValues && !validValues.includes(envVar)) {
        return defaultValue;
    }
    return envVar;
}
 ...
 */
export function buildCompletionCriteriaFromEnv() {
    return {
        tier1: {
 ...
 */
export function buildAuditTimeoutConfigFromEnv() {
    return {
 ...
 */
export function buildConcurrencyConfigFromEnv() {
    return {
 ...
 */
export function buildSynchronousConfigFromEnv() {
    return {
        ...
        feedbackDetailLevel: parseEnvString(process.env.SYNC_AUDIT_FEEDBACK_DETAIL_LEVEL, 'detailed', ['basic', 'detailed', 'verbose']),
 ...
 */
export function mergeRuntimeConfig(base, overrides) {
    return {
        completionCriteria: {
...
//# sourceMappingURL=synchronous-config.js.map
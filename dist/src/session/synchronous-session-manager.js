...
import { SessionError, SessionNotFoundError, SessionCorruptionError, } from '../types/error-types.js';
import { handleSessionCorruption, handleSessionError, } from '../utils/error-handler.js';
const execFileAsync = promisify(execFile);
/**
 * Default configuration for SynchronousSessionManager
 */
export const DEFAULT_SYNCHRONOUS_SESSION_MANAGER_CONFIG = {
 ...
 */
export class SynchronousSessionManager extends SessionManager {
    syncConfig;
    syncComponentLogger;
    activeContexts = new Map(); // loopId -> contextId
    constructor(config = {}) {
     ...
     */
    async createEnhancedSession(id, config, loopId) {
        try {
            const now = Date.now();
            const session = {
                id,
            ...
            this.syncComponentLogger.error(`Failed to create enhanced session ${id}`, error);
            throw error;
        }
    }
    /**
    ...
    async startCodexContext(loopId) {
        try {
            ...
            const { stdout } = await execFileAsync(this.syncConfig.codexExecutable, ['context', 'start', '--loop-id', loopId], { timeout: this.syncConfig.codexTimeout });
            ...
            this.syncComponentLogger.error(`Failed to start Codex context for loop ${loopId}`, error);
    ...
    async maintainCodexContext(loopId, contextId) {
        try {
            ...
            await execFileAsync(this.syncConfig.codexExecutable, ['context', 'maintain', '--context-id', contextId, '--loop-id', loopId], { timeout: this.syncConfig.codexTimeout });
            this.syncComponentLogger.debug(`Maintained Codex context ${contextId} for loop ${loopId}`);
        }
        catch (error) {
            this.syncComponentLogger.error(`Failed to maintain Codex context ${contextId}`, error);
    ...
    async terminateCodexContext(loopId, reason) {
        ...
        try {
            await execFileAsync(this.syncConfig.codexExecutable, ['context', 'terminate', '--context-id', contextId, '--reason', reason], { timeout: this.syncConfig.codexTimeout });
            this.activeContexts.delete(loopId);
            this.syncComponentLogger.info(`Terminated Codex context ${contextId} for loop ${loopId} (reason: ${reason})`);
        }
        catch (error) {
            this.syncComponentLogger.error(`Failed to terminate Codex context ${contextId}`, error);
    ...
    async getSession(id) {
        try {
            ...
            if (!validation.isValid && validation.recoverable) {
                ...
                const corruptionError = new SessionCorruptionError(id, validation.corruptionType, [
                    'Automatic recovery failed',
                    'Create a new session with default configuration',
                    'Contact support if this issue persists',
                ]);
                const result = await handleSessionCorruption(corruptionError, id, validation.corruptionType);
            ...
            this.syncComponentLogger.error(`Error loading session ${id}`, error);
    ...
    async validateSessionIntegrity(sessionId) {
        try {
            ...
            this.syncComponentLogger.error(`Failed to validate session ${sessionId}`, error);
            return {
                isValid: false,
     ...
     */
    async recoverCorruptedSession(sessionId, corruptionType) {
        ...
        try {
            ...
            this.syncComponentLogger.error(`Session recovery failed for ${sessionId}`, error);
            return null;
        }
    }
    /**
     ...
     */
    async recoverMissingFields(session) {
        const now = Date.now();
        return {
     ...
     */
    async recoverFormatMismatch(session) {
     ...
     */
    async recoverPartialData(session) {
        const recovered = await this.recoverMissingFields(session);
        // Filter out corrupted iterations
        if (session.iterations && Array.isArray(session.iterations)) {
            recovered.iterations = session.iterations.filter((iter) => iter && iter.thoughtNumber && iter.code && iter.auditResult);
        }
     ...
     */
    async recoverDataInconsistency(session) {
        const recovered = await this.recoverMissingFields(session);
     ...
     */
    async terminateAllContexts(reason = 'manual') {
        ...
        const terminationPromises = activeContexts.map(loopId => this.terminateCodexContext(loopId, reason).catch(error => {
            this.syncComponentLogger.error(`Failed to terminate context for loop ${loopId}`, error);
        }));
    ...
    async cleanupStaleContexts() {
        ...
        for (const [loopId, contextId] of this.activeContexts.entries()) {
            try {
                ...
                await execFileAsync(this.syncConfig.codexExecutable, ['context', 'status', '--context-id', contextId], { timeout: 5000 } // Short timeout for status check
                );
            }
            catch (error) {
    ...
    async handleSessionTimeout(sessionId) {
        try {
            ...
            this.syncComponentLogger.error(`Failed to handle session timeout for ${sessionId}`, error);
        }
    }
    /**
     ...
     */
    async handleSessionFailure(sessionId, error) {
        try {
            const session = await this.getSession(sessionId);
            if (session?.loopId && session.codexContextActive) {
            ...
            this.syncComponentLogger.error(`Failed to handle session failure for ${sessionId}`, cleanupError);
        }
    }
    /**
    ...
    destroy() {
        ...
        this.terminateAllContexts('manual').catch(error => {
            this.syncComponentLogger.error('Failed to cleanup contexts during destroy', error);
        });
        super.destroy();
    }
}
//# sourceMappingURL=synchronous-session-manager.js.map
 ...
 */
import { SynchronousSessionManager, DEFAULT_SYNCHRONOUS_SESSION_MANAGER_CONFIG } from './synchronous-session-manager.js';
import { createComponentLogger } from '../utils/logger.js';
import { gzipSync, gunzipSync } from 'zlib';
/**
 * Default configuration for memory-efficient session manager
 */
export const DEFAULT_MEMORY_EFFICIENT_CONFIG = {
...
export class MemoryEfficientSessionManager extends SynchronousSessionManager {
    memoryConfig;
    memoryLogger;
    compressedIterations = new Map();
    memoryMonitoringTimer;
    memoryStats;
    constructor(config = {}) {
    ...
    async optimizeAllSessions() {
        ...
        try {
            ...
            this.memoryLogger.error('Memory optimization failed', error);
        }
    }
    /**
    ...
    async forceCleanup() {
        ...
        try {
            ...
            this.memoryLogger.error('Force cleanup failed', error);
        }
    }
    /**
     * Get detailed memory breakdown by session
     */
    async getMemoryBreakdown() {
        const breakdown = [];
        try {
            const sessionIds = await this.getAllSessionIds();
            for (const sessionId of sessionIds) {
            ...
            this.memoryLogger.error('Failed to get memory breakdown', error);
        }
        return breakdown;
    }
    /**
     ...
     */
    async optimizeSessionMemory(sessionId) {
        const session = await super.getSession(sessionId);
     ...
     */
    async compressOldIterations(sessionId, iterations) {
     ...
     */
    async trimIterations(sessionId, iterations) {
     ...
     */
    decompressIteration(compressed) {
     ...
     */
    async decompressSessionIterations(session) {
     ...
     */
    calculateIterationSize(iteration) {
        const iterationJson = JSON.stringify(iteration);
        ...
        this.memoryMonitoringTimer = setInterval(() => {
            ...
            if (this.memoryStats.totalMemoryUsage > this.memoryConfig.maxMemoryUsage) {
                this.performEmergencyCleanup().catch(error => {
                    this.memoryLogger.error('Emergency cleanup failed', error);
                });
            }
 ...
 */
export function createMemoryEfficientSessionManager(config = {}) {
    return new MemoryEfficientSessionManager(config);
}
//# sourceMappingURL=memory-efficient-session-manager.js.map
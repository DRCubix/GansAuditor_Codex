 ...
 */
export const DEFAULT_COMPLETION_TIERS = [
    {
 ...
 */
export function validateSystemPromptConfig(config) {
    const errors = [];
    const warnings = [];
 ...
 */
export function mergeSystemPromptConfig(config) {
    return {
 ...
 */
export function loadSystemPromptConfigFromFile(filePath, options = {}) {
    const opts = {
        strict: false,
 ...
 */
export function saveSystemPromptConfigToFile(config, filePath, options = {}) {
    const opts = {
 ...
 */
export function createSystemPromptConfigFile(filePath, environment = 'development') {
 ...
 */
function parseEnvInteger(value, defaultValue, min, max) {
    if (!value)
        return defaultValue;
 ...
 */
function parseEnvFloat(value, defaultValue, min, max) {
    if (!value)
        return defaultValue;
 ...
 */
function parseEnvString(value, defaultValue, validValues) {
    if (!value)
        return defaultValue;
 ...
 */
export function loadSystemPromptConfigFromEnv() {
    const config = {};
    ...
    if (process.env.GAN_AUDITOR_STANCE) {
        config.identity = {
            ...DEFAULT_SYSTEM_PROMPT_CONFIG.identity,
            ...config.identity,
            stance: parseEnvString(process.env.GAN_AUDITOR_STANCE, DEFAULT_SYSTEM_PROMPT_CONFIG.identity.stance, ['adversarial', 'collaborative', 'constructive-adversarial'])
        };
    }
    if (process.env.GAN_AUDITOR_AUTHORITY) {
        config.identity = {
            ...DEFAULT_SYSTEM_PROMPT_CONFIG.identity,
            ...config.identity,
            authority: parseEnvString(process.env.GAN_AUDITOR_AUTHORITY, DEFAULT_SYSTEM_PROMPT_CONFIG.identity.authority, ['spec-and-steering-ground-truth', 'flexible', 'advisory'])
        };
    }
    ...
    if (process.env.GAN_AUDITOR_WEIGHTING_SCHEME) {
        config.qualityFramework = {
            ...DEFAULT_SYSTEM_PROMPT_CONFIG.qualityFramework,
            ...config.qualityFramework,
            weightingScheme: parseEnvString(process.env.GAN_AUDITOR_WEIGHTING_SCHEME, DEFAULT_SYSTEM_PROMPT_CONFIG.qualityFramework.weightingScheme, ['project-standard', 'custom', 'balanced'])
        };
    }
    if (process.env.GAN_AUDITOR_SCORING_SCALE) {
        config.qualityFramework = {
            ...DEFAULT_SYSTEM_PROMPT_CONFIG.qualityFramework,
            ...config.qualityFramework,
            scoringScale: parseEnvString(process.env.GAN_AUDITOR_SCORING_SCALE, DEFAULT_SYSTEM_PROMPT_CONFIG.qualityFramework.scoringScale, ['0-100', '0-10', 'letter-grade'])
        };
    }
    if (process.env.GAN_AUDITOR_AGGREGATION_METHOD) {
        config.qualityFramework = {
            ...DEFAULT_SYSTEM_PROMPT_CONFIG.qualityFramework,
            ...config.qualityFramework,
            aggregationMethod: parseEnvString(process.env.GAN_AUDITOR_AGGREGATION_METHOD, DEFAULT_SYSTEM_PROMPT_CONFIG.qualityFramework.aggregationMethod, ['weighted-average', 'minimum', 'geometric-mean'])
        };
    }
 ...
 */
export function loadSystemPromptConfig(configFilePath, options = {}) {
    const sources = {
        defaults: true,
        configFile: false,
        environment: false,
 ...
 */
export function getSystemPromptConfigSummary(config) {
    return {
 ...
 */
export function generateEnvVarDocumentation() {
    const sections = [
        {
...
//# sourceMappingURL=system-prompt-config.js.map
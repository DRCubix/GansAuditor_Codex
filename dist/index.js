...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    try {
        ...
        const errorResponse = createValidationError(`Unknown tool: ${request.params.name}`, `The requested tool '${request.params.name}' is not supported by this server`, {
...
async function runServer() {
    ...
    if (envConfig.enableGanAuditing) {
        ...
        try {
            ...
            const errorResponse = createCodexError('Codex CLI validation failed during server startup', validationError instanceof Error ? validationError.message : String(validationError), {
                phase: 'startup',
